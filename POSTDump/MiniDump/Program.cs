...
namespace Minidump
{
    public class Program
    {
        ...
        public static void Main(IntPtr baseaddr, long size, string file = "")
        {
            BinaryReader data;
            ...
            if (file.Equals(String.Empty)) { 
                unsafe
                {
                    ...
                    using (UnmanagedMemoryStream memoryStream = new UnmanagedMemoryStream(memBytePtr, size, size, FileAccess.Read))
                    {
                        memoryStream.Seek(0, SeekOrigin.Begin);
                        memoryStream.Read(buffer, 0, (int)size);
                        memoryStream.Flush();
                        memoryStream.Close();
                    }
                ...
                data = new BinaryReader(Memstream);
            }
            else
            {
                Filestream = File.Open(file, FileMode.Open);
                data = new BinaryReader(Filestream);
            }
